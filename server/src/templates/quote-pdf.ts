...
export function renderQuoteHtml(data: {
  ...
  const statusLabel = quote.status.replace('_', ' ').toUpperCase();
  const statusColor = quote.status === 'accepted' ? '#16a34a'
    : quote.status === 'expired' ? '#dc2626'
    : quote.status === 'sent' ? '#2563eb' : '#6b7280';
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .totals-total {
      ...
      border-top: 2px solid #7c3aed; padding-top: 8px;
    }
    .notes-section {
...
<body>
  <div class="container">
    ...
    <div class="meta-grid">
      ...
      <div style="text-align: right;">
        <div style="margin-bottom: 12px;">
          ...
          <div class="meta-value">${formatDate(quote.issueDate)}</div>
        </div>
        ${quote.expiryDate ? `
        <div style="margin-bottom: 12px;">
          <div class="meta-label">Valid Until</div>
          <div class="meta-value">${formatDate(quote.expiryDate)}</div>
        </div>` : ''}
      </div>
    </div>
    ...
    <div style="display: flex; flex-direction: column; align-items: flex-end;">
      ...
      <div class="totals-row">
        <div class="totals-label">Tax (${quote.taxRate}%)</div>
        <div class="totals-value">
          ${formatCurrency(quote.taxAmount, currency)}
        </div>
      </div>` : ''}
      ...
      <div class="totals-row">
        ...
        <div class="totals-value" style="color: #dc2626;">
          -${formatCurrency(quote.discountAmount, currency)}
        </div>
      </div>` : ''}
      <div class="totals-row">
